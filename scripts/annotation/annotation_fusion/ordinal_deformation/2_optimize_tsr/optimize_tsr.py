...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir, os.path.pardir, os.path.pardir, os.path.pardir, 'util')))
from util import GetTSRSumSquareError, NormedDiff, TikzplotlibFixNCols
import agreement_metrics as agree

def OptimizeTSR(signal_path, tsr_path, output_path, subj_id_signal_regex='(\d+)_average\.csv', subj_id_tsr_regex='(\d+)_tsr_T.*\.csv', seg_tsr_regex='.*_tsr_T(\d+)\.csv', file_name_filter_str='', show_plots=False):
   ...
   if file_name_filter_str != '':
      signal_files = [x for x in signal_files if file_name_filter_str in x]
      tsr_files = [x for x in tsr_files if file_name_filter_str in x]
   ...
   for subj_id in subj_tsr_error_dict.keys():
      ...
      if show_plots:
         ...
         tikzplotlib.save(tikz_output_file, figure=fig)
...
if __name__ == '__main__':
   ...
   parser.add_argument('--file_name_filter_str', required=False, type=str, default='', help='A substring used to filter the view of files living in the input_signal_path and input_tsr_path.  Only files containing this substring will be used during execution')
   ...
   try:
      ...
      parser.print_help()
      sys.exit(0)
   OptimizeTSR(args.input_signal_path, args.input_tsr_path, args.output_path, args.subj_id_regex, args.subj_id_tsr_regex, args.segment_num_regex, args.file_name_filter_str, args.show_plots)
