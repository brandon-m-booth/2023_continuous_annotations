...
def MediaevalFusion():
   ...
   for anno_dimension in [('arousal', arousal_anno_path), ('valence', valence_anno_path)]:
      print("Fusing %s annotations..."%(anno_dimension[0]))
      ...
      # Debug-only code
      #anno_files = [x for x in anno_files if '1761' in x]
      ...
      for anno_file_idx in range(len(anno_files)):
         ...
         if not subject_id in subject_ids:
            print('Ignoring subject %s'%(subject_id))
            continue
         if subject_id in skip_subj_ids:
         ...
         max_jobs = 16
         opt_strategy = 'elbow'
         best_x, best_y, best_cost, best_num_segs = tsr.ComputeOptimalFit(average_signal_csv, min_segments, max_segments, max_jobs, out_file_path_1, opt_strategy=opt_strategy)
