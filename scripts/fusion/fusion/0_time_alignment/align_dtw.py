...
def AlignDTW(input_anno_dir, output_anno_dir, max_warp_seconds=5, show_plots=False):
   ...
   for input_anno in input_annos:
      ...
      dtw_df['Time(sec)'] = dtw_df.index*10.0
      dtw_df_cols = dtw_df_cols.insert(0, 'Time(sec)')
      dtw_df = dtw_df[dtw_df_cols]

      if show_plots:
         for anno_col in dtw_df.columns[1:]:
            ax.plot(dtw_df['Time(sec)'], dtw_df[anno_col])
         plt.title(os.path.basename(input_anno) + ' DTW annotations')
         plt.show()

      output_file_name = os.path.basename(input_anno)
      dtw_df.to_csv(os.path.join(output_anno_dir, output_file_name), index=False, header=True)
...
if __name__=='__main__':
   ...
   try:
      ...
      parser.print_help()
      sys.exit(0)
   AlignDTW(args.input_annos, args.output_path, args.max_warp_seconds, args.show_plots)
